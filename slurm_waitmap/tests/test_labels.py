...
def test_define_labels():
    ...
    ##non-positive values
    with pytest.raises(ValueError):
        define_labels([0, 2], [1, 2])
    with pytest.raises(ValueError):
        define_labels([1, 2], [-1, 2])
    ##duplicate/non-ascending values
    with pytest.raises(ValueError):
        define_labels([1, 1, 2], [1, 2])
    with pytest.raises(ValueError):
        define_labels([2, 1], [1, 2])
    ...
    assert time_labels == ["<1", "1-<2", "2-<24", "24-<48", "48+"]
